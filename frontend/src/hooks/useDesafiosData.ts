...
export function useDesafiosData() {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [liberacoesRes, desafiosRes, conteudosRes] = await Promise.all([
          ...
          apiJson<Desafio[]>(`/api/desafios?cpf=${cpf}`),
          apiJson<{ id: string; modulo: string; aula: string }[]>(`/api/conteudos`)
        ]);

        const liberadosIds = new Set(liberacoesRes.map(l => l.conteudo_id));
        const conteudosMap = new Map<string, { id: string; modulo: string; aula: string }>(conteudosRes.map(c => [c.id, c]));
  ...
  // 'useMemo' otimiza a performance, agrupando os desafios por módulo apenas quando a lista de desafios muda.
